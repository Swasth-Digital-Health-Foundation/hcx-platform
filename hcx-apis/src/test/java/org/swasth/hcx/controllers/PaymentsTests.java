...
public class PaymentsTests extends BaseSpec {
    ...
    public void check_payment_request_success_scenario() throws Exception {
        ...
        MvcResult mvcResult = mockMvc.perform(post(Constants.PAYMENT_NOTICE_REQUEST).content(requestBody).contentType(MediaType.APPLICATION_JSON)).andReturn();
    ...
    public void check_payment_on_request_success_scenario() throws Exception {
        doNothing().when(mockKafkaClient).send(anyString(),anyString(),any());
        String requestBody = getRequestBody();
        MvcResult mvcResult = mockMvc.perform(post(Constants.PAYMENT_NOTICE_ONREQUEST).content(requestBody).contentType(MediaType.APPLICATION_JSON)).andReturn();
        MockHttpServletResponse response = mvcResult.getResponse();
        int status = response.getStatus();
        assertEquals(202, status);
    }
