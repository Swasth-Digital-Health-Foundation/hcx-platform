...
public class ClaimsTests extends BaseSpec {
  ...
  public void check_claim_submit_success_scenario() throws Exception {
      ...
      MvcResult mvcResult = mockMvc.perform(post(Constants.CLAIM_SUBMIT).content(requestBody).contentType(MediaType.APPLICATION_JSON)).andReturn();
  ...
  public void check_claim_on_submit_success_scenario() throws Exception {
      doNothing().when(mockKafkaClient).send(anyString(),anyString(),any());
      String requestBody = getRequestBody();
      MvcResult mvcResult = mockMvc.perform(post(Constants.CLAIM_ONSUBMIT).content(requestBody).contentType(MediaType.APPLICATION_JSON)).andReturn();
      MockHttpServletResponse response = mvcResult.getResponse();
      int status = response.getStatus();
      assertEquals(202, status);
  }
