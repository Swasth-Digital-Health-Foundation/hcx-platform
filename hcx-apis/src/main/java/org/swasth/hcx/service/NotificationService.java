...
public class NotificationService {
    ...
    public void processOnSubscription(Request request, Response response) throws Exception {
        ...
        if (updateSubscription(subscriptionId, statusCode)) {
            LOG.info("Subscription record updated for subscriptionId:" + subscriptionId);
        } else {
            LOG.info("Subscription record is not updated for subscriptionId:" + subscriptionId);
        ...
        String subscriptionMessage = eventGenerator.generateSubscriptionEvent(request.getMid(), request.getApiAction(), subscription.getRecipient_code(), request.getSenderCode());
        ...
        senderList.stream().forEach(senderCode -> {
            try {
                if (!senderCode.equalsIgnoreCase(hcxRegistryCode)) {
                    String subscriptionMessage = eventGenerator.generateSubscriptionEvent(request.getMid(), request.getApiAction(), senderCode, request.getNotificationRecipientCode());
