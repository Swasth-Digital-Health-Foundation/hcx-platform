...
public class HCXValidationFilter extends AbstractGatewayFilterFactory<HCXValidationFilter.Config> {
        ...
        return (exchange, chain) -> {
            ...
            try {
                ...
                if (path.contains(NOTIFICATION_NOTIFY)) {
                    //validating notify api request
                    processNotifyReq(requestBody, path, requestObj);
                } else if (requestBody.containsKey(PAYLOAD)) {
                    //validating jwe payload request
                    processPayloadReq(requestBody, path, requestObj, correlationId, apiCallId, subject);
                } else if (path.contains(NOTIFICATION_SUBSCRIBE) || path.contains(NOTIFICATION_UNSUBSCRIBE)) {
                    //validating notification subscribe and unsubscribe request
                    processSubscriptionReq(requestBody, path, requestObj, subject);
                } else if (path.contains(NOTIFICATION_SUBSCRIPTION_LIST)) {
                    //validating notification subscription list request
                    processSubscriptionListReq(requestBody, path, requestObj, subject);
                } else if (path.contains(NOTIFICATION_SUBSCRIPTION_UPDATE) || path.contains(NOTIFICATION_ON_SUBSCRIBE)) {
                    //validating notification subscription update and on subscribe request
                    processSubscriptionUpdateReq(requestBody, path, requestObj, subject);
                } else {
                    //validating redirect and error plain JSON on_check calls
                    processJsonReq(requestBody, path, requestObj, correlationId, apiCallId, subject);
                }
            } catch (Exception e) {
    ...
    private void processNotifyReq(Map<String, Object> requestBody, String path, BaseRequest requestObj) throws Exception {
    ...
    private void processPayloadReq(Map<String, Object> requestBody, String path, BaseRequest requestObj, String correlationId, String apiCallId, String subject) throws Exception {
    ...
    private void processSubscriptionReq(Map<String, Object> requestBody, String path, BaseRequest requestObj, String subject) throws Exception {
    ...
    private void processSubscriptionListReq(Map<String, Object> requestBody, String path, BaseRequest requestObj, String subject) throws Exception {
        JSONRequest jsonRequest = new JSONRequest(requestBody, true, path, hcxCode, hcxRoles);
        requestObj = jsonRequest;
        Map<String, Object> recipientDetails = registryService.fetchDetails(OS_OWNER, subject);
        requestObj.getPayload().put(RECIPIENT_CODE, recipientDetails.get(PARTICIPANT_CODE));
    ...
    private void processSubscriptionUpdateReq(Map<String, Object> requestBody, String path, BaseRequest requestObj, String subject) throws Exception {
        JSONRequest jsonRequest = new JSONRequest(requestBody, true, path, hcxCode, hcxRoles);
        requestObj = jsonRequest;
    ...
    private void processJsonReq(Map<String, Object> requestBody, String path, BaseRequest requestObj, String correlationId, String apiCallId, String subject) throws Exception {
    ...
    private List<String> getErrorMandatoryHeaders() {
        return env.getProperty("plainrequest.headers.mandatory", List.class);
    }
    ...
    private List<String> getRedirectMandatoryHeaders() {
        return env.getProperty("redirect.headers.mandatory", List.class);
    }

    private List<String> getApisForRedirect() {
        return env.getProperty("redirect.apis", List.class);
    }

    private List<String> getRolesForRedirect() {
        return env.getProperty("redirect.roles", List.class);
    }

    private List<String> getSubscriptionMandatoryHeaders() {
        return env.getProperty("notification.subscription.headers.mandatory", List.class);
    }
