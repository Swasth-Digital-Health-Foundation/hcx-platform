...
public class ClaimsStreamTaskTest {
    ...
    BaseJobConfig baseJobConfig = new BaseJobConfig(config, "ClaimsTestJob");
    FlinkKafkaConnector mockKafkaUtil = mock(new FlinkKafkaConnector(baseJobConfig).getClass());

    @Before
    public void beforeClass() throws Exception {
        when(mockKafkaUtil.kafkaMapSource(claimsConfig.kafkaInputTopic)).thenReturn(new ClaimsSource());
        flinkCluster.before();
    }
    ...
    @Test
    public void testClaimsTask() throws Exception {
        ClaimsStreamTask task = new ClaimsStreamTask(claimsConfig, mockKafkaUtil);
    ...
    private static class ClaimsSource implements SourceFunction<Map<String,Object>> {
