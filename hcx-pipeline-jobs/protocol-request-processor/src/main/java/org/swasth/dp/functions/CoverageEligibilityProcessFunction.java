...
public class CoverageEligibilityProcessFunction extends BaseDispatcherFunction {
    ...
    private final Logger logger = LoggerFactory.getLogger(CoverageEligibilityProcessFunction.class);
    private ProtocolRequestProcessorConfig config;

    public CoverageEligibilityProcessFunction(ProtocolRequestProcessorConfig config) {
        super(config);
        this.config = config;
    }
