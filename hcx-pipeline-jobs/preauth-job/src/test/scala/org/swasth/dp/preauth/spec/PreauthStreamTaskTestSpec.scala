...
import org.swasth.dp.preauth.task.{PreauthConfig, PreauthStreamTask}
...
class PreauthStreamTaskTestSpec extends BaseTestSpec {
  ...
  "Preauth" should "validate the success flow" in {
    ...
    val task = new PreauthStreamTask(eligibilityCheckConfig, mockKafkaUtil)
    task.process()
