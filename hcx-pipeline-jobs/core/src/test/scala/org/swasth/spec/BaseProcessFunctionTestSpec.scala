...
class BaseProcessFunctionTestSpec extends BaseSpec with Matchers {
  ...
  "Validation of SerDe" should "validate serialization and deserialization of Map, String and Event schema" in {
    ...
    val mapStream =
      env.fromSource(kafkaConnector.kafkaMapSource(bsConfig.kafkaMapInputTopic), WatermarkStrategy.noWatermarks[mutable.Map[String, AnyRef]](), "map-event-consumer")
  ...
  def retrieveMid(message: String): String = {
    gson.fromJson(message, new util.HashMap[String, AnyRef]().getClass).get("mid").asInstanceOf[String]
  }
