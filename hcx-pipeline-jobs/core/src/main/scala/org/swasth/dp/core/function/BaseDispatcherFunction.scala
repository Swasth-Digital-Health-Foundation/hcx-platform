...
abstract class BaseDispatcherFunction (config: BaseJobConfig)
  ...
  def dispatchErrorResponse(event: util.Map[String, AnyRef], error: Option[ErrorResponse], senderCtx: util.Map[String, AnyRef], context: ProcessFunction[util.Map[String, AnyRef], util.Map[String, AnyRef]]#Context, metrics: Metrics): Unit = {
    ...
    //Generate new UUID for api call id for each request
  ...
  override def processElement(event: util.Map[String, AnyRef], context: ProcessFunction[util.Map[String, AnyRef], util.Map[String, AnyRef]]#Context, metrics: Metrics): Unit = {
    ...
    try {
      ...
      case ex: PipelineException =>
        //TODO modify the code to send across only up to 500 characters in the trace
        val errorResponse = ErrorResponse(Option(ex.code), Option(ex.message), Option(ex.trace))
        dispatchErrorResponse(event, ValidationResult(true, Option(errorResponse)).error, senderCtx, context, metrics)
    }
  }
