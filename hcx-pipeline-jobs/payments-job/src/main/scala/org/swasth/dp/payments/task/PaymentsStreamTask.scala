...
class PaymentsStreamTask(config: PaymentsConfig, kafkaConnector: FlinkKafkaConnector) {
...
object PaymentsStreamTask {
  def main(args: Array[String]): Unit = {
    val configFilePath = Option(ParameterTool.fromArgs(args).get("config.file.path"))
    val config = configFilePath.map {
      path => ConfigFactory.parseFile(new File(path)).resolve()
    }.getOrElse(ConfigFactory.load("payments.conf").withFallback(ConfigFactory.systemEnvironment()))
    val pipelinePreprocessorConfig = new PaymentsConfig(config)
    val kafkaUtil = new FlinkKafkaConnector(pipelinePreprocessorConfig)
    val task = new PaymentsStreamTask(pipelinePreprocessorConfig, kafkaUtil)
    task.process()
  }
}
