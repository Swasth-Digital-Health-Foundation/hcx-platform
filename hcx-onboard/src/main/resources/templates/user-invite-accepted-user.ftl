<html>
<body>
Dear ${USER_NAME},<br/><br/>
Congratulations! We are pleased to inform you that you have successfully completed the user onboarding process for role ${ROLE} for ${PARTICIPANT_NAME} on the HCX platform.<br/><br/>
Your user_id will be : ${USER_ID} </br></br>
Please use your assigned username and password to log in at the following link: <a href="https://${ENV}-hcx.swasth.app/onboarding/login">SSP-Login</a>.<br/><br/>
Our support team is available at <a href="mailto:Operations_admin@swasthalliance.org">Operations_admin@swasthalliance.org</a> to address any queries or if you require any further information.<br/><br/>
We wish you the best of luck in your new role.<br/><br/>
Best regards,<br/>
HCX Onboarding Team
</body>
</html>